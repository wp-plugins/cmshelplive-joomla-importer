...
class j2wpcmshelplive{
		...
		public function dispatch() {
			...
			$options = get_option('j2wpcmshelplive_options');
			...
			if ( !is_writable($upload_dir['basedir']) ) {
				...
				$this->display_admin_error(__('The wp-content directory must be writable.', 'j2wpcmshelplive'));
			...
			if ( isset($_POST['empty']) ) {
				...
				if ( check_admin_referer( 'empty', 'j2wpcmshelplive_nonce' ) ) { // Security check
			...
			elseif ( isset($_POST['save']) ) {
				...
				$this->display_admin_notice(__('Settings saved', 'j2wpcmshelplive'));
			...
			elseif ( isset($_POST['test']) ) {
				...
				if ( check_admin_referer( 'parameters_form', 'j2wpcmshelplive_nonce' ) ) { // Security check
			...
			elseif ( isset($_POST['remove_cat_prefix']) ) {
				...
				if ( check_admin_referer( 'remove_cat_prefix', 'j2wpcmshelplive_nonce' ) ) { // Security check

					$result = $this->remove_category_prefix();

					$this->display_admin_notice(__('Prefixes removed from categories', 'j2wpcmshelplive'));
			...
			elseif ( isset($_POST['modify_links']) ) {
				...
				if ( check_admin_referer( 'modify_links', 'j2wpcmshelplive_nonce' ) ) { // Security check
		...
		public function add_help_tab() {
			...
			$screen->add_help_tab(array(
				...
				'id'	=> 'j2wpcmshelplive_help_instructions',
			...
			$screen->add_help_tab(array(
				...
				'id'	=> 'j2wpcmshelplive_help_options',
			...
			$screen->set_help_sidebar(__('<a href="http://wordpress.org/plugins/fg-joomla-to-wordpress/faq/" target="_blank">FAQ</a>'), 'j2wpcmshelplive');
		...
		protected function joomla_connect() {
			...
			if ( !class_exists('PDO') ) {
				...
				$this->display_admin_error(__('PDO is required. Please enable it.', 'j2wpcmshelplive'));
			...
			try {
				...
				$this->display_admin_error(__('Couldn\'t connect to the Joomla database. Please check your parameters. And be sure the WordPress server can access the Joomla database.', 'j2wpcmshelplive') . '<br />' . $e->getMessage());
		...
		public function joomla_query($sql) {
			...
			try {
				...
				$this->display_admin_error(__('Error:', 'j2wpcmshelplive') . $e->getMessage());
		...
		private function empty_database($action) {
			...
			if ( $action == 'all' ) {
				...
				update_option('j2wpcmshelplive_start_id', $start_id);
...
SQL;
		...
		function test_database_connection() {
			...
			if ( $this->joomla_connect() ) {
				...
				try {
					...
					$this->display_admin_notice(__('Connected with success to the Joomla database', 'j2wpcmshelplive'));

					

					do_action('j2wpcmshelplive_post_test_database_connection');
		...
		public function test_joomla_1_0() {
			...
			if ( version_compare($this->plugin_options['version'], '1.5', '<') ) {
				...
				$this->display_admin_error(__('Your version of Joomla (probably 1.0) is not supported by this plugin. Please consider upgrading to the <a href="http://www.fredericgilles.net/fg-joomla-to-wordpress/" target="_blank">Premium version</a>.', 'j2wpcmshelplive'));

				// Deactivate the Joomla info feature

				remove_action('j2wpcmshelplive_post_test_database_connection', array($this, 'get_joomla_info'), 9);
		...
		public function get_joomla_info() {
			...
			$message = __('Joomla data found:', 'j2wpcmshelplive') . '<br />';
			...
			if ( version_compare($this->plugin_options['version'], '1.5', '<=') ) {
				...
				$message .= sprintf(_n('%d section', '%d sections', $sections_count, 'j2wpcmshelplive'), $sections_count) . '<br />';
			...
			$message .= sprintf(_n('%d category', '%d categories', $cat_count, 'j2wpcmshelplive'), $cat_count) . '<br />';
			...
			$message .= sprintf(_n('%d article', '%d articles', $posts_count, 'j2wpcmshelplive'), $posts_count) . '<br />';
			...
			$message .= sprintf(_n('%d user', '%d users', $users_count, 'j2wpcmshelplive'), $users_count) . '<br />';
			...
			$message .= sprintf(_n('%d web link', '%d web links', $weblinks_count, 'j2wpcmshelplive'), $weblinks_count) . '<br />';

			

			$message = apply_filters('j2wpcmshelplive_pre_display_joomla_info', $message);
		...
		private function save_plugin_options() {
			...
			update_option('j2wpcmshelplive_options', $this->plugin_options);

			

			// Hook for doing other actions after saving the options

			do_action('j2wpcmshelplive_post_save_plugin_options');
		...
		private function import() {
			...
			if ( $this->joomla_connect() ) {
				...
				$do_import = apply_filters('j2wpcmshelplive_pre_import_check', true);
				...
				do_action('j2wpcmshelplive_pre_import');
				...
				if ( !isset($this->premium_options['skip_categories']) || !$this->premium_options['skip_categories'] ) {
					...
					$this->display_admin_notice(sprintf(_n('%d category imported', '%d categories imported', $cat_count, 'j2wpcmshelplive'), $cat_count));
				...
				if ( !isset($this->premium_options['skip_articles']) || !$this->premium_options['skip_articles'] ) {
					...
					switch ($this->post_type) {
						...
						case 'page':
							...
							$this->display_admin_notice(sprintf(_n('%d page imported', '%d pages imported', $result['posts_count'], 'j2wpcmshelplive'), $result['posts_count']));
						...
						default:
							...
							$this->display_admin_notice(sprintf(_n('%d post imported', '%d posts imported', $result['posts_count'], 'j2wpcmshelplive'), $result['posts_count']));

					}

					$this->display_admin_notice(sprintf(_n('%d media imported', '%d medias imported', $result['media_count'], 'j2wpcmshelplive'), $result['media_count']));
					...
					if ($this->post_type == 'post') {
						...
						if ( $this->plugin_options['meta_keywords_in_tags'] ) {
							...
							$this->display_admin_notice(sprintf(_n('%d tag imported', '%d tags imported', $result['tags_count'], 'j2wpcmshelplive'), $result['tags_count']));
				...
				do_action('j2wpcmshelplive_post_import');

				

				// Hook for other notices

				do_action('j2wpcmshelplive_import_notices');
				...
				//$this->display_admin_notice(__("Don't forget to modify internal links.", 'j2wpcmshelplive'));
		...
		private function import_categories() {
			...
			if ( is_array($categories) ) {
				...
				foreach ( $categories as $category ) {
					...
					$new_category = apply_filters('j2wpcmshelplive_pre_insert_category', $new_category, $category);
					...
					do_action('j2wpcmshelplive_post_insert_category', $cat_id, $category);
					...
					if ( $cat ) {
						...
						if ( !empty($category['parent']) ) {
							...
							if ( $parent_cat ) {
								...
								$cat = apply_filters('j2wpcmshelplive_pre_edit_category', $cat, $parent_cat);

								wp_update_term($cat->term_id, 'category', array('parent' => $parent_cat->term_id));

								// Hook after editing the category

								do_action('j2wpcmshelplive_post_edit_category', $cat);
				...
				do_action('j2wpcmshelplive_post_import_categories', $categories);
		...
		private function import_posts() {
			...
			do_action('j2wpcmshelplive_pre_import_posts');
				...
				if ( is_array($posts) ) {
					...
					foreach ( $posts as $post ) {
						...
						if ( ($this->plugin_options['archived_posts'] == 'not_imported') && (in_array($post['state'], array(-1, 2))) ) {
							...
							update_option('j2wpcmshelplive_last_joomla_id', $post['id']);
						...
						$post = apply_filters('j2wpcmshelplive_pre_process_post', $post);
						...
						if ( !$this->plugin_options['skip_media'] ) {
							...
							list($featured_image, $post) = apply_filters('j2wpcmshelplive_pre_import_media', array($featured_image, $post));
						...
						$categories = apply_filters('j2wpcmshelplive_post_categories', $categories, $post);
						...
						$new_post = apply_filters('j2wpcmshelplive_pre_insert_post', $new_post, $post);
						...
						if ( $new_post_id ) {
							...
							add_post_meta($new_post_id, '_j2wpcmshelplive_old_id', $post['id'], true);
							...
							do_action('j2wpcmshelplive_post_insert_post', $new_post_id, $post);
			...
			do_action('j2wpcmshelplive_post_import_posts');
		...
		private function get_sections() {
			...
			try {
				...
				$sql = apply_filters('j2wpcmshelplive_get_sections_sql', $sql, $prefix);
				...
				$sections = apply_filters('j2wpcmshelplive_get_sections', $sections);
		...
		private function get_categories() {
			...
			try {
				...
				$sql = apply_filters('j2wpcmshelplive_get_categories_sql', $sql, $prefix);
				...
				$categories = apply_filters('j2wpcmshelplive_get_categories', $categories);
		...
		protected function get_posts($limit=1000) {
			...
			$last_joomla_id = (int)get_option('j2wpcmshelplive_last_joomla_id'); // to restore the import where it left
			...
			try {
				...
				$extra_cols = apply_filters('j2wpcmshelplive_get_posts_add_extra_cols', '');

				$extra_joins = apply_filters('j2wpcmshelplive_get_posts_add_extra_joins', '');
				...
				$sql = apply_filters('j2wpcmshelplive_get_posts_sql', $sql, $prefix, $cat_field, $extra_cols, $extra_joins, $last_joomla_id, $limit);
		...
		private function remove_category_prefix() {
			...
			do_action('j2wpcmshelplive_pre_remove_category_prefix');
			...
			do_action('j2wpcmshelplive_post_remove_category_prefix');
		...
		public function get_imported_joomla_posts() {
			...
			$sql = "SELECT post_id, meta_value FROM {$wpdb->postmeta} WHERE meta_key = '_j2wpcmshelplive_old_id'";
